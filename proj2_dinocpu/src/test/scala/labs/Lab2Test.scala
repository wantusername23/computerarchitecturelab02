...
/**
  ...
  * {{{
  * sbt 'testOnly dinocpu.ALUControlTesterLab2'
  * }}}
  */
...
/**
  ...
  * From within sbt use:
  * {{{
  * testOnly dinocpu.SingleCycleAddTesterLab2
  * }}}
  * From a terminal shell use:
  * {{{
  * sbt 'testOnly dinocpu.SingleCycleAddTesterLab2'
  * }}}
  */
class SingleCycleAddTesterLab2 extends CPUFlatSpec {
...
/**
  ...
  * From within sbt use:
  * {{{
  * testOnly dinocpu.SingleCycleMultiCycleTesterLab2
  * }}}
  * From a terminal shell use:
  * {{{
  * sbt 'testOnly dinocpu.SingleCycleMultiCycleTesterLab2'
  * }}}
  *
  * To run a **single** test from this suite, you can use the -z option to sbt test.
  * The option after the `-z` is a string to search for in the test
  * {{{
  * sbt> testOnly dinocpu.SingleCycleMultiCycleTesterLab2 -- -z swapxor
  * }}}
  * Or, to run just the r-type instructions you can use `-z rtype`
  */
class SingleCycleMultiCycleTesterLab2 extends CPUFlatSpec {
  behavior of "Single Cycle CPU"
